...
public class TaskCatalystCommons {
	...
	public static String interpretedString(String userInput) {
		...
		for (DateGroup dateGroup : dateGroups) {
			...
			// This fixes the partial matching problem, for almost everything!
			// OMG!
			int position = dateGroup.getPosition();
			int length = dateGroup.getText().length();
			int startIndex = Math.max(0, position - 5);
			int endIndex = Math.min(position + length + 5,
					parsingInput.length());
			boolean wholeWord = parsingInput.substring(startIndex, endIndex)
					.matches(
							".*(^|\\b)" + Pattern.quote(dateGroup.getText())
									+ "(\\b|$).*");
			...
			if (longMatch && wholeWord) {
				...
				// blackBox.fine("RegExp: " + "(^|\\b)" + matchingText +
				// "(\\b|$)" + onlyOutsideBrackets);
